...
export class WorkMonthDetailItem extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            workMonth: {},
    ...
    componentDidMount = () => {
        const { workMonth, model } = this.props;
        if (!workMonth) return;
    ...
    render = () => {
        const { workMonth, mode, model } = this.state;
        return (
            <>
                    ...
                    <Card.Body>
                        <div className="w-30 pl-4 pt-3">
                            <label className="w-100">
                                Năm, tháng chấm công:
                                <input fieldname="yearMonth" value={model.yearMonth} onChange={this.onInputChange} disabled={mode === Mode.EDIT || mode === Mode.VIEW} className="form-control" placeholder="Năm, tháng chấm công"></input>
                            </label>
                            <label className="w-100 mt-2">
                                Ngày bắt đầu:
                                <CustomDatePicker value={model.startDate} disabled={mode === Mode.VIEW} onDateChange={this.onValidDateChange} />
                            </label>
                            <label className="w-100 mt-2 text-camelcase">
                                Ngày kết thúc:
                                <CustomDatePicker value={model.endDate} disabled={mode === Mode.VIEW} onDateChange={this.onValidDateChange} />
                            </label>
                            <label className="w-100 mt-2 text-camelcase">
                                Số công trong tháng:
                                <input fieldname="totalWork" type="number" value={model.totalWork} onChange={this.onInputChange} disabled={mode === Mode.VIEW} className="form-control" placeholder="Số công trong tháng"></input>
                            </label>
    ...
    onProcessRemoveConfirm =() => {
        const {model} = this.state;
        // CALL_API
        // WorkMonthServices.DeleteWorkMonthItem(model.id)
        // .then(response => {
    ...
    onProcessConfirm =() => {
        const {model, mode, workMonth} = this.state;
        if(mode === Mode.CREATE)
        {
            const newModel = Object.assign({},{...model, functionId: workMonth.id , createdBy: AuthenticationManager.UserName()});
            // CALL_API
            // WorkMonthServices.AddWorkMonthItem(newModel)
            // .then(response => {
        ...
        }else if(mode === Mode.EDIT)
        {
            ...
            // CALL_API
            // WorkMonthServices.UpdateWorkMonthItem(editModel.id, editModel)
            // .then(response =>{
