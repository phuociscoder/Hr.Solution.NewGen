...
export class NatiProvDiscWardList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            categoryItems: [],
    ...
    componentDidMount = () => {
        const { category, prefix, idTypeCategory } = this.props;
        if (!category) return;
        this.setState({ loading: true, prefix: prefix, idTypeCategory: idTypeCategory }, this.loadCategoryItems(category.id));
    }

    loadCategoryItems = (categoryId) => {
        const { idTypeCategory } = this.state;
        ...
        // if (idTypeCategory) {
        //     this.setState({ categoryItems: this.filterCategoryItemsByIdTypeCategory(idTypeCategory)});
        // }
    }
    ...
    shouldComponentUpdate = (nextProps) => {
        ...
        if (this.props.prefix !== nextProps.prefix) {
            this.setState({ prefix: nextProps.prefix }, this.loadCategoryItems(nextProps.category.id));
        }
        if (this.props.idTypeCategory !== nextProps.idTypeCategory) {
            this.setState({ idTypeCategory: nextProps.idTypeCategory }, this.loadCategoryItems(nextProps.category.id));
        }
    ...
    filterCategoryItemsByIdTypeCategory = (idType) => {
        const { originCategoryItems } = this.state;
        // const filteredItems = originCategoryItems.filter(x => )
    }
