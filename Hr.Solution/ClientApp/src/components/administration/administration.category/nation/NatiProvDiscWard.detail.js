...
export class NatiProvDiscWardDetails extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            category: {},
    ...
    componentDidMount = () => {
        const { category, model, prefix } = this.props;
        if (!category) return;
        this.setState({ category: category });
        if (Object.keys(model).length > 0) {
            this.setState({ model: model, editModel: model, mode: Mode.EDIT, prefix: prefix });
        }
    }
    ...
    shouldComponentUpdate = (nextProps) => {
        ...
        if(this.props.prefix !== nextProps.prefix && Object.keys(nextProps.prefix).length > 0)
        {
            this.setState({prefix: nextProps.prefix, editModel: nextProps.prefix, mode: Mode.EDIT});
        }
        return true;
    }
